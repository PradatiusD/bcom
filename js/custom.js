jQuery(function ($) {
  ...
  $(window).on('load resize orientationchange', function () {
    ...
    var ImageStretcher = {
      getDimensions: function (data) {
        ...
        const ratio = data.imageRatio || (data.imageWidth / data.imageHeight)
      ...
      add: function (options) {
        const container = $(options.container ? options.container : window)
        const image = typeof options.image === 'string' ? container.find(options.image) : $(options.image)
  ...
  const byRow = $('body').hasClass('js-ready')
  ...
  teamList.each(function () {
    $(this).owlCarousel({
      items: $(this).find('.item-wrap').length,
      itemsDesktop: [1199, 5],
      itemsDesktopSmall: [979, 4],
      itemsTablet: [768, 3],
      itemsTabletSmall: true,
